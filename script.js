...
let colorDict = {'Blue':'#3D5588FF', 'Yellow':'#FFE800FF','Pink':'#FF48B0FF',
                    'Red':'#F15060FF', 'Teal': '#00838A'};
let satDict = {'twenty':'33', 'forty':'66', 'sixty':'99', 'eighty':'CC',
                 'hundred':'FF'}; //saturation dictionary
let val = 'FF'

//Changes the color with your mouse
function colorTrail() {
    if (this.style.background == ""){ // if there is no color there add it
        this.style.background = color;
        this.setAttribute('colorID', color)
    } else{ // otherwise blend the current color ('color')with what's there
        let existingColor = this.getAttribute('colorID')
        let val2 = existingColor.slice(-2)
        let mixedColor = "#";
        for(let i = 0; i<3; i++) { //from programming-idioms.org
...
saturationButtons.forEach(saturation => saturation.addEventListener('click',
    changeMouseSaturation))
